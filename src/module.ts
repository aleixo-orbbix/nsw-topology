...
import { DEFAULT_GENERAL, DEFAULT_APPEARANCE, DEFAULT_COLORS, DEFAULT_INTERACTION } from './constants';

export const plugin = new PanelPlugin<TopologyOptions>(TopologyPanel).setNoPadding().setPanelOptions((builder) => {
  builder
    .addTextInput({
      path: 'general.title',
      name: 'Title',
    ...
    .addBooleanSwitch({
      path: 'interaction.editMode',
      name: 'Edit Mode',
      description:
        'When off, the canvas is read-only: the toolbar is hidden and nodes/connections cannot be moved, resized, created, edited or deleted.',
      defaultValue: DEFAULT_INTERACTION.editMode,
